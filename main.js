...
window.addEventListener('mousedown', function(e){
    ...
    if ((e.clientX < xWindow) || (e.clientY > yWindow)){
        ...
        if (planetChecklist.mercury || planetChecklist.venus || 
            planetChecklist.earth || planetChecklist.mars ||
            planetChecklist.jupiter || planetChecklist.saturn ||
            planetChecklist.uranus || planetChecklist.neptune
             == true) {
            altPlanetsLabelOn()
        }    
    }  
...
function generateOrbit(radius){
    const orbitPoints = new THREE.Path().absarc(0, 0, radius, 0, 2 * Math.PI).getPoints(500)
...
function generatePlanet(name, size, position, image, alt, ring){
    ...
    var ringMaterial = 0
    var labelDiv = 0
    if (!alt){var material = new THREE.MeshPhongMaterial({map: texture})}
    else {var material = new THREE.MeshBasicMaterial({map: texture})}
...
//generate planets
const mercury = generatePlanet("Mercury", 0.04 * dim.sunSize, 1 * dim.mercuryDist, 'mercury.jpg', false)
const venus = generatePlanet("Venus", 0.09 * dim.sunSize, 1.9 * dim.mercuryDist, 'venus.jpg', false)
const earth = generatePlanet("Earth", 0.10 * dim.sunSize, 2.6 * dim.mercuryDist, 'earth.jpg', false)
const mars = generatePlanet("Mars", 0.05 * dim.sunSize, 4 * dim.mercuryDist, 'mars.jpg',false)
const jupiter = generatePlanet("Jupiter", 0.25 * dim.sunSize, 5.5 * dim.mercuryDist, 'jupiter.jpg')
const saturn = generatePlanet("Saturn", 0.23 * dim.sunSize, 7.2 * dim.mercuryDist, 'saturn.jpg', false, {
    innerRadius: dim.sunSize/4,
    outerRadius: dim.sunSize/2,
    texture: 'saturnring.png'
}) 
const uranus = generatePlanet("Uranus", 0.18 * dim.sunSize, 8.4 * dim.mercuryDist, 'uranus.jpg', false, {
    innerRadius: dim.sunSize/6,
    outerRadius: dim.sunSize/3,
    texture: 'uranusring.png'
})
const neptune = generatePlanet("Neptune", 0.17 * dim.sunSize, 9.9 * dim.mercuryDist, 'neptune.jpg', false)

//alt planets
const mercuryAlt = generatePlanet("Mercury", 0.04 * dim.sunSize, 1 * dim.mercuryDist, 'mercury.jpg', true)
const venusAlt = generatePlanet("Venus", 0.09 * dim.sunSize, 1.9 * dim.mercuryDist, 'venus.jpg', true)
const earthAlt = generatePlanet("Earth", 0.10 * dim.sunSize, 2.6 * dim.mercuryDist, 'earth.jpg', true)
const marsAlt = generatePlanet("Mars", 0.05 * dim.sunSize, 4 * dim.mercuryDist, 'mars.jpg', true)
const jupiterAlt = generatePlanet("Jupiter", 0.25 * dim.sunSize, 5.5 * dim.mercuryDist, 'jupiter.jpg', true)
const saturnAlt = generatePlanet("Saturn", 0.23 * dim.sunSize, 7.2 * dim.mercuryDist, 'saturn.jpg', true, {
    innerRadius: dim.sunSize/4,
    outerRadius: dim.sunSize/2,
    texture: 'saturnring.png'
}) 
const uranusAlt = generatePlanet("Uranus", 0.18 * dim.sunSize, 8.4 * dim.mercuryDist, 'uranus.jpg', true, {
    innerRadius: dim.sunSize/6,
    outerRadius: dim.sunSize/3,
    texture: 'uranusring.png'
})
const neptuneAlt = generatePlanet("Neptune", 0.17 * dim.sunSize, 9.9 * dim.mercuryDist, 'neptune.jpg', true)
altPlanetsOff()
...
planetFolder.add(planets, 'options', [
    'default', 'mercury', 'venus', 
    'earth', 'mars', 'jupiter', 
    'saturn', 'uranus', 'neptune'
]).name('view').listen().onChange(newValue => setChecklist(newValue))
planetFolder.open()
...
//gui label
    if (planetLabel.label){
        sunLabelDiv.style.opacity = 1
        if (mercury.obj.visible){mercury.label.style.opacity = 1}
        else {mercury.label.style.opacity = 0}
        if (venus.obj.visible){venus.label.style.opacity = 1}
        else {venus.label.style.opacity = 0}
        if (earth.obj.visible){earth.label.style.opacity = 1}
        else {earth.label.style.opacity = 0}
        if (mars.obj.visible){mars.label.style.opacity = 1}
        else {mars.label.style.opacity = 0}
        if (jupiter.obj.visible){jupiter.label.style.opacity = 1}
        else {jupiter.label.style.opacity = 0}
        if (saturn.obj.visible){saturn.label.style.opacity = 1}
        else {saturn.label.style.opacity = 0}
        if (uranus.obj.visible){uranus.label.style.opacity = 1}
        else {uranus.label.style.opacity = 0}
        if (neptune.obj.visible){neptune.label.style.opacity = 1}
        else {neptune.label.style.opacity = 0}
        if (mercuryAlt.obj.visible){mercuryAlt.label.style.opacity = 1}
        else {mercuryAlt.label.style.opacity = 0}
        if (venusAlt.obj.visible){venusAlt.label.style.opacity = 1}
        else {venusAlt.label.style.opacity = 0}
        if (earthAlt.obj.visible){earthAlt.label.style.opacity = 1}
        else {earthAlt.label.style.opacity = 0}
        if (marsAlt.obj.visible){marsAlt.label.style.opacity = 1}
        else {marsAlt.label.style.opacity = 0}
        if (jupiterAlt.obj.visible){jupiterAlt.label.style.opacity = 1}
        else {jupiterAlt.label.style.opacity = 0}
        if (saturnAlt.obj.visible){saturnAlt.label.style.opacity = 1}
        else {saturnAlt.label.style.opacity = 0}
        if (uranusAlt.obj.visible){uranusAlt.label.style.opacity = 1}
        else {uranusAlt.label.style.opacity = 0}
        if (neptuneAlt.obj.visible){neptuneAlt.label.style.opacity = 1}
        else {neptuneAlt.label.style.opacity = 0}
    }
...
//tilt rotation 
    ...
    saturnAlt.ringmesh.rotateOnAxis(saturnTilt, 0.22 * dim.earthRot * dim.slowRate)
    uranusAlt.mesh.rotateOnAxis(uranusTilt, 1.4 * dim.earthRot * dim.slowRate)
    uranusAlt.ringmesh.rotateOnAxis(uranusTilt, 0.14 * dim.earthRot * dim.slowRate)
...
function planetsOn(name){
    ...
    if (name !== undefined){name.obj.visible = false}
}
...
function altPlanetsOff(name){
    ...
    if (name !== undefined){name.obj.visible = true}
}
...
function planetsLabelOn(name){
    ...
    if (name !== undefined){name.label.style.opacity = 0}
}
...
function planetsLabelOff(name){
    ...
    uranus.label.style.opacity = 0
    neptune.label.style.opacity = 0
    if (name !== undefined){name.label.style.opacity = 1}
}
